public class Main {
	...
	private static void printFormatNumber(int lengh){
		...
		String[][] deleteGroups = getGroupsForThreeNumber(lastNumber);
		String[][] deleteGroups2 = getGroupsForThreeNumber(lastNumber2);

		int lastTotal = getAddTotal(lastNumber, lengh);
		System.out.println(lastTotal);
		for (int i = 0; i < maxNumber; i++) {
			String number = getStringNumber(i, lengh);
			if (
					hasRepeatCount(number) ||
					isDeleteNumber(number, lengh, Numbers.mDeleteNumbers) ||
//					isDeleteGroup(number, Numbers.mDeleteGroups, lengh) ||
					isHistoryNumber(number, Numbers.mDeleteHistories) ||
					isAddNumberEquals(number, lastTotal, lengh) ||
//					isAddEqualsForBaiNumber(number) ||
//					isAddEqualsForGeNumber(number) ||
//					isTwoNumberAddEqualsLast(number, lastNumber, TYPE_QIAN2) ||
					isTwoNumberAddEqualsLast(number, lastNumber, TYPE_HOU2) ||
					isDeleteGroup(number, deleteGroups2, lengh) || 
					isDeleteGroup(number, deleteGroups, lengh)
					) {
	...
	private static final int TYPE_QIAN2 = 1;
	private static final int TYPE_HOU2 = 2;

	private static boolean isTwoNumberAddEqualsLast(String number, String lastNumber, int type){
